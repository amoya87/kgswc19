...
Furthermore, we show in Figure~\ref{fig:result} that effectively the size of the window influences the quality of the results when considering \textsc{r}, being better as the window increases. However, these results are not affected when we consider only predicates in \textsc{q} or  \textsc{p}. Also, we show, that all configurations outperform the random baseline.
